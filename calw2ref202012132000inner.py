...
def Check_location(fits_file):
    ...
    if np.nanstd(fitsimg.data[0,0,:,:]) < 1000.0:
        ...
        with open(r'/zfs/helios/filer0/idayan/calw2ref-202012132000/try_fit_results_.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(fields)
        
        if slope_cor < 1e8:
            
            #print os.path.basename(fits_file)
            ...
            fitsimg.writeto("/zfs/helios/filer0/idayan/calw2ref-202012032122/"+filename,overwrite=True)
...
fits_list=sorted(glob.glob(obs_dir+"202012132000/"+"*_all/"+"SB*/"+"imgs/"+"*.fits"))
...
Parallel(n_jobs=15,backend="multiprocessing", verbose=10)(delayed(Check_location)(i) for i in arranged_fits_list)
